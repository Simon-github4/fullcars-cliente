...
public class CarpartExcelExporter implements Exporter<CarPart> {
    ...
    public byte[] export(List<CarPart> parts) throws IOException {
        try (Workbook workbook = new SXSSFWorkbook(); ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            String[] headers = {"SKU", "Nombre", "Descripcion", "Stock", "Marca", "Modelo", "Proveedor", "Categoria", "PrecioBase"};
            ...
            for (CarPart p : parts) {
                ...
                row.createCell(4).setCellValue(p.getModel().getBrand().getName() != null ? p.getModel().getBrand().getName() : "");
                row.createCell(5).setCellValue(p.getModel() != null ? p.getModel().getName() : "");
